...
function dibujarTabla()
{
	...
	for (f=0;f<30;f++)
	{
		...
		for (c=0;c<30;c++)
		{
			...
			creandoTD.addEventListener("mousedown", function(){activarPintar(this)}, false);		//evento para pintar al hacer clic
			creandoTD.addEventListener("mouseover", function(){pintar(this)}, false);				//evento para pintar moviendo el raton
			creandoTD.addEventListener("mouseup", function(){desactivarPintar()}, false);			//evento para parar de pintar
...
function prepararSelecciones()
{
	...
	{
		colores[i].id = i;
		colores[i].addEventListener("click", function(){seleccionado(this)}, false);
	}
}
...
{	
	...
	switch(color.id)									// cambia el className del color seleccionado 
      {
        case "0":
...
function activarPintar(td)
{
	...
	if(estadoPintar = true){               // se puede pintar
...
function pintar(td)
{
 	if (estadoPintar == true)			//mientras pintar este activado pinta por donde pasa el raton
	{
